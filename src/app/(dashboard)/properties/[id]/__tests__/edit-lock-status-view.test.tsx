...
describe("仕様6.3の表: 物件が他の人の鍵のとき止まる4つ", () => {
  ...
  it("④ 所在検索の地番保存を無効化する(RegistryLocationSearchButtonにeditLockHeldを渡す)", () => {
    // ⚠固定の文字数窓は、他のprops(コメント込み)が数行増えるだけで正しいコードの
    //   まま赤くなる。この画面には<RegistryLocationSearchButtonが1箇所しか無い
    //   ため、unboundedでも安全(save-entrypoints-scan.test.tsと同じ判断)。
    expect(PROPERTY_DETAIL_PAGE_SRC).toMatch(
      /<RegistryLocationSearchButton[\s\S]*?editLockHeld=\{propertyEditLockHeld\}/,
    );
  });
...
describe("仕様6.3の表: 所有者Nが鍵のとき止まるのはそのカードだけ", () => {
  ...
  it("OwnerCardの「所有者情報を編集」ボタンだけを無効化する", () => {
