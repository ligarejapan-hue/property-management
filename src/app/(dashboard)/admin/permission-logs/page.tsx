  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
      <nav className="mb-4 text-sm text-gray-500 dark:text-gray-400">
        ...
        <span className="text-gray-900 dark:text-gray-100">権限変更履歴</span>
      </nav>

      <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6">権限変更履歴</h1>
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
            <thead className="bg-gray-50 dark:bg-gray-800/50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">操作者</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">対象ユーザー</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">変更種別</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">変更内容</th>
              </tr>
            </thead>
...
function ChangeDetail({ oldValue, newValue }: { oldValue: unknown; newValue: unknown }) {
  if (!oldValue && !newValue) return <span className="text-gray-400 dark:text-gray-500">—</span>;
  ...
  if (added.length === 0 && removed.length === 0) {
    return <span className="text-gray-400 dark:text-gray-500">変更なし</span>;
  }

  return (
    <div className="space-y-1">
      {added.map((p: { resource: string; action: string }, i: number) => (
        <span key={`add-${i}`} className="inline-flex mr-1 rounded bg-green-50 dark:bg-green-500/10 px-1.5 py-0.5 text-xs text-green-700 dark:text-green-300">
          +{p.resource}:{p.action}
        </span>
      ))}
      {removed.map((p: { resource: string; action: string }, i: number) => (
        <span key={`rm-${i}`} className="inline-flex mr-1 rounded bg-red-50 dark:bg-red-500/10 px-1.5 py-0.5 text-xs text-red-700 dark:text-red-300">
          -{p.resource}:{p.action}
        </span>
      ))}
    </div>
