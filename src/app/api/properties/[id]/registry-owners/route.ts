...
export async function POST(
  ...
  try {
    ...
    const result = await processRegistryPdf({
      ...
      ownersOnly: true,
      // ⚠**既知の制限(発注者判断 2026-09-22 で許容)**: 所有者の登録が確定した
      //   あとに行う後始末(取込ジョブの記録・監査の記録)が失敗すると、
      //   **所有者は入っているのにエラーを返す**。やり直すと 409 になる。
      //   ただし所有者は物件画面に出ており、ボタンも消えるので
      //   「入ったことが分からなくなる」状態にはならない(表示を読み直せば分かる)。
      //   直すには後始末も同じトランザクションに入れる必要があり、
      //   共有部分(手動取込・有料取得)の作りを変えることになるため見送った。
    });
