...
export async function POST(request: Request) {
  try {
    ...
    for (const s of stale) {
      ...
      const upd = await prisma.fieldSurveySession.updateMany({
        ...
        data: {
          ...
          // ⚠**自動終了した時点では踏破マップに出さない**(@codex #356 P1)。
          // 無操作1時間で終了させたということは「終了ボタンが押されていない」
          // = **本人はまだ歩いているかもしれない**(圏外で送信できていないだけ)。
          // ここで出すと、それまでに届いていた経路が即座に他スタッフへ共有され、
          // 実行中の巡回を隠す守りを自動終了が回避してしまう。
          // 出すのは「本人が終了を押した」か「さらに1時間なにも起きなかった」
          // のどちらかを確認できてから (下の確定処理)。
    ...
    // ⚠**踏破マップへの復帰**(@codex #356 P1)。自動終了した巡回は「本人はまだ
    // 歩いているかもしれない」ので、いったん踏破マップから外してある
    // (自動終了した時点 + その後に位置記録が届いた時点の両方)。
    // その巡回が**さらに**無操作1時間を超えたら、今度こそ歩き終えたと判断して
    // 印を外し、終了時刻も最後に記録した時刻に直す。
    // ⚠ここを作らないと、自動終了した巡回が**二度と踏破マップに出ない**
    // = 二度歩きを避けるという機能の目的そのものを損なう。
    // (本人が終了ボタンを押せば、この1時間を待たずに戻る。)
