...
const src = readFileSync(join(dir, "..", "photo-tab.tsx"), "utf8");

describe("photo-tab.tsx dark: 配色 (物件詳細タブ群)", () => {
  // --- エラーバナー accent 可読化 ---
  ...
  // --- drag-active accent ---
  it("drag-active 時の bg-blue-50 に dark:bg-blue-500/15 がある", () => {
  ...
  it("text-gray-800 に dark:text-gray-100 がある", () => {
    expect(src).toContain("dark:text-gray-100");
  });
