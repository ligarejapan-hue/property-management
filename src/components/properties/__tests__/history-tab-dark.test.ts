...
const src = readFileSync(join(dir, "..", "history-tab.tsx"), "utf8");

describe("history-tab.tsx dark: 配色 (物件詳細タブ群)", () => {
  // --- 面（背景）---
  it("テーブルヘッダ/フィルタ bg-gray-50 に dark:bg-gray-800/50 がある", () => {
    expect(src).toContain("dark:bg-gray-800/50");
  });
  it("リセットボタン bg-white に dark:bg-gray-900 がある", () => {
    expect(src).toContain("dark:bg-gray-900");
  });
  it("ソースバッジ bg-gray-100 に dark:bg-gray-800 がある", () => {
  ...
  it("text-gray-300（null値）に dark:text-gray-600 がある", () => {
  ...
  // --- 入力欄 3点セット ---
  it("入力欄 select/input に dark:text-gray-100 がある", () => {
  ...
  // --- 差分表示（変更前=赤/変更後=緑）accent 可読化 ---
  it("変更前値 bg-red-50 に dark:bg-red-500/10 がある", () => {
    expect(src).toContain("dark:bg-red-500/10");
  });
  it("変更前値 text-red-700 に dark:text-red-400 がある", () => {
    expect(src).toContain("dark:text-red-400");
  });
  it("変更後値 bg-green-50 に dark:bg-green-500/10 がある", () => {
    expect(src).toContain("dark:bg-green-500/10");
  });
  it("変更後値 text-green-700 に dark:text-green-400 がある", () => {
  ...
  it("ライトモード text-green-700 は残っている", () => {
