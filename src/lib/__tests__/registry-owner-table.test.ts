...
describe("parseRegistryOwnerTable（サービスの定型文が混ざる実物のケース）", () => {
  ...
  it("⚠氏名の続きが数字だけでも、刷り込みの目印より前なら氏名としてつなげる", () => {
    // 法人名の末尾の数字が折り返して1セルになることがある。刷り込み(整理番号)は
    // 実物2,500本で必ず「…番号 ####-##-#」の目印の後に出た(目印なしは0件)ので、
    // 目印より前の数字だけのセルは氏名の続きとして残す。
    const numericTail = [
      "┏━━━━━━━━━┓",
      "┃ 所 有 者 ┃",
      "┠─────┬────┨",
      "┃ 住 所 │ 氏 名 ┃",
      "┠─────┼────┨",
      "┃東京都中央区銀座一丁目1番1号 │合同会社サンプルパートナーズ ┃",
      "┃ │２０２１ ┃",
      "┃ │ 検索用資料番号 0801-01-0┃",
      "┃ │ 14425 ┃",
      "┗━━━━━┷━━━━┛",
    ].join("\n");
    const owners = parseRegistryOwnerTable(numericTail);
    expect(owners).toHaveLength(1);
    expect(owners?.[0]?.name).toBe("合同会社サンプルパートナーズ２０２１");
  });
...
describe("所有者の表の終わり", () => {
  it("⚠下枠の後に別の表(甲区など)が続いても、その行を所有者にしない", () => {
    // 全部事項や手動で上げた謄本には、所有者の表の後に「順位番号│登記の目的│…」の
    // 表が続く。下枠で読み取りを終えないと、列数が足りる行が全部「所有者」になる
    // (氏名「登記の目的」「所有権保存」の所有者が作られる)。
    // 実物4,000本では下枠は必ず1つで、その後にセルの行は0件(複数ページの20本も、
    // 見出しの繰り返しは下枠より前に出る)=下枠で終えても続きを取りこぼさない。
    const withNextTable = [
      "┏━━━━━━━━━━━━┓",
      "┃ 所 有 者 ┃",
      "┠──────┬──────┨",
      ...
      "┠────┼──────┼──────┼────────┨",
      "┃1 │所有権保存 │平成20年1月1日第1号 │所有者 東京都港区赤坂一丁目1番1号 鈴木一郎 ┃",
      "┗━━━━┷━━━━━━┷━━━━━━┷━━━━━━━━┛",
    ].join("\n");
    const owners = parseRegistryOwnerTable(withNextTable);
    expect(owners).toEqual([
      { name: "山田太郎", address: "東京都渋谷区神宮前三丁目12番3号", share: null },
    ]);
  });

  it("下枠より前に見出しが繰り返されても(ページ送り)、続きの所有者を読む", () => {
    const twoPages = [
      "┏━━━━━━━━━━━━┓",
      "┃ 所 有 者 ┃",
      "┠──────┬──────┨",
...
describe("住所の続きが数字だけの行", () => {
  it("⚠数字だけの続き（部屋番号など）を捨てない", () => {
    // サービスの刷り込み(整理番号)を除く仕掛けを住所にも当てると、
    // 「101」のような続きが消えて住所が切れてしまう。
    const numeric = [
      "┏━━━━━━━━━━━┓",
      "┃ 所 有 者 ┃",
      "┠──────┬──────┨",
      "┃ 住 所 │ 氏 名 ┃",
      "┠──────┼──────┨",
      "┃東京都杉並区高円寺南四丁目1番2号ハイツ杉並 │山田太郎 ┃",
      "┃101 │ ┃",
      "┗━━━━━━┷━━━━━━┛",
    ].join(String.fromCharCode(10));

    const owners = parseRegistryOwnerTable(numeric);
    expect(owners).toHaveLength(1);
    expect(owners?.[0]?.address).toBe(
      "東京都杉並区高円寺南四丁目1番2号ハイツ杉並101",
    );
  });
});
