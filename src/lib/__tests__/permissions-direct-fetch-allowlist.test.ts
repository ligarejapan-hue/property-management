 ...
 *   一方で properties 詳細 / admin owner 詳細 / field-survey map は依然
 *   `fetch("/api/me/permissions")` を直接持つ（= 次に provider へ寄せる候補）。
 *
...
describe("19-A: /api/me/permissions 直接 fetch 許可リスト（source assertion）", () => {
  ...
  it("直接 fetch の総 call site 数は 4（各許可ファイル 1 箇所ずつ）", () => {
  ...
  it("移行待ち 3 ページは現状なお直接 fetch を保持（次の F12 展開候補）", () => {
