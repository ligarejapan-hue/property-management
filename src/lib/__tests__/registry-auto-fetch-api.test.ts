...
describe("【回収】購入済みの謄本を再課金なしで取り込む(mode:recover)", () => {
  ...
  it("⚠所在だけが書き換わっていても取り込まない(versionの一致だけに頼らない防御)", async () => {
    // version の一致確認は取得を開始した瞬間しか見ない(TOCTOU)。数分かかる取得の
    // 最中に所在が書き換わると、探す区域が変わり**別の物件の書類**を取り込みかねない。
    // この検査は「version を上げない書込経路がある」ことに頼らず、実際に使う値
    // (所在・地番)そのものを直接比べる defense-in-depth(@codex #394 R23 P1。
    // Task 9 で「version を上げない書込経路」自体は洗い出して閉じたが、
    // この防御は引き続き必要)。
    setProperty({
