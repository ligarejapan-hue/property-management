/**
 * source-assertion: DELETE /api/properties/[id]/photos/[photoId] が
 * DB 削除成功後に storage.delete を best-effort で呼び、失敗しても
 * 既存成功レスポンスを維持し、fileUrl / key を console / AuditLog に
 * 生値で残さないことを保証する。
 *
 * 既存 project の route テストは prisma を mock しない source-assertion 方式
 * (import-rollback-route-phase2.test.ts と同パターン) を採用しているため
 * 本テストも同方式に揃える。
 */
...
describe("PropertyPhoto DELETE — storage cleanup", () => {
