...
const NON_PERSONAL_LABELS_EXACT: ReadonlySet<string> = new Set(
  [
    ...
    // ⚠**自由記述の見出しはここに載せない**(@codex PR#414 20巡目)。
    //   R15 では「サンプルAの コメント を備考へ届けたい」を理由に載せていたが、
    //   自由記述は**何でも書けるフィールド**であり、それを「本質的に非個人」と
    //   分類するのは原理的に誤りだった(形で拾えない個人情報はラテン文字の氏名に
    //   限らない: 勤務先・家族構成など)。
    //   いまの withheld は画面に案内付きで表示され、人が1操作で備考へ移せる＝
    //   「壊れる」ではなく「1手増える」。大原則
    //   「過剰に伏せる誤りは回復可能、漏らす誤りは回復不能」に例外を作らない。
    //   → 自由記述は**値の判定に落とす**(値が安全確定なら備考、それ以外は withheld)。
  ].map((l) => l.normalize("NFKC").replace(/[\s　]/g, "").toUpperCase()),
);
