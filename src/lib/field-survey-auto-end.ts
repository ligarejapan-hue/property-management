...
/**
 * 自動終了した巡回を、**本人が終了ボタンを押して確定させた**印
 * (@codex #356 P2)。
 *
 * ⚠これが無いと「自動終了した巡回」の印が永久に残り、**確定した後に届いた
 * 古い送信まで受け入れて**しまう(開きっぱなしの別タブ等)。確定済みの巡回に
 * 後から座標が足されると、記録が伸び、踏破マップからも再び消える。
 *
 * ⚠見回りによる確定ではこの印にしない。見回りの確定は「1時間なにも起きな
 * かった」という**推測**でしかないため、その後に圏外から復帰した端末の送信は
 * 受け入れる必要がある(受け入れれば、また確認待ちに戻って自然に落ち着く)。
 * 人が押したときだけが「もう歩いていない」と言い切れる。
 */
export const TRIP_AUTO_END_CONFIRMED_REASON = "idle_timeout_confirmed";
