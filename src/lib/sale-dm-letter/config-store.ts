...
// DB→env フォールバック・絶対http(s)検証は resolveTrackingBaseUrl に委譲(print/t と同一規則)。
// ⚠外部LPの住所(lpUrl)はここでは返さない。転送先が要る経路(/t/ の転送)は loadSaleDmLpUrl を使う。
//   ページ描画に使わない値を読んで返すと、読む列が増えた理由が分からなくなる(この関数の存在意義は
//   「描画に要る最小限しか読まない」こと)。
