...
//
// 右境界の厳格化(Codex 追加 P2):
//   末尾に negative lookahead `(?![0-9０-９A-Za-z])` を付け、捕捉した数字列の直後に
//   数字(半角/全角)または ASCII 英字が連結している場合は「12桁で完結していない」
//   トークンとみなして部分一致させない。これが無いと "会社法人等番号 020001012345A"
//   や全角 "…０１２３４５abc" で先頭12桁だけを捕捉し、normalize が妥当な会社法人等番号と
//   誤認 → 部分抽出/部分除去(壊れた残骸 "…A" 等)を起こす。
//   ・直後がさらに数字 → 13桁以上に化けるトークン全体を捕捉し normalize(12桁検証)で弾く。
//   ・直後が空白/区切り/和文/末尾 → 従来どおり 12桁で完結したラベル値として採用する。
//   ハイフン類は値の内部表記(0200-01-012345)で許容するため右境界の除外対象に含めない。
const LABELED_COMPANY_REGISTRY_NUMBER_RE =
  /(?:会社法人等番号|法人等番号)[\s:：=]*([0-9０-９][0-9０-９\s\-‐‑‒–—―ー－−─]{10,30})(?![0-9０-９A-Za-z])/gi;
