...
describe("registryPdfBulkStagingKey", () => {
  it("ジョブID/行番号からキーを組み立てる", () => {
    expect(registryPdfBulkStagingKey("job-uuid-1", 3)).toBe(
      "import-staging/registry-pdf/job-uuid-1/3.pdf",
    );
  });
  ...
  it("不正な入力は throw(キーにtraversal要素を入れない)", () => {
    ...
    expect(() => registryPdfBulkStagingKey("job", 0)).toThrow();
    expect(() => registryPdfBulkStagingKey("job", 1.5)).toThrow();
  });
});
```
