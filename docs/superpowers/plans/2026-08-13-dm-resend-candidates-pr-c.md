...
> ⚠**この節は @codex #374 R1 P1 を受けて訂正済み**。当初は「§4-3（期間）だけ」と書いていたが、その根拠が誤っていた。

控え作成〜ダウンロードの間に変わりうる §4 の条件のうち、

- §4-1 dmStatus / archived → 既存の検査(3)が見ている
- §4-2 送付記録が1件以上 → 記録が消えても「送りすぎ」にはならない（初回DMを送るのと同じ）
- **§4-3 90日以内の送付が無い → どこも見ていない。「二重に送る」事故になる**
- **§4-4 のうち `replied`（連絡あり）→ どこも見ていない。**⚠既存の検査(2)は **`refused`/`undeliverable` しか見ない**ので、「拒否・宛先不明は検査(2)が見ている」は正しいが **`replied` は漏れる**。連絡が来た相手を定型の再送に乗せないのが §4 の意図なので、ここで止める
- §4-4 のうち拒否・宛先不明 / §4-5 所有者横断 → 既存の検査(2)が見ている

ので、検査(5) は **§4-3 と §4-4（`SELF_EXCLUDING_REACTION_VALUES` 全部）** を再評価する。§4-1/§4-2 を見ないのは**意図的**（非対称の原則で、送りすぎだけを止める）。

**教訓:** 「既存の検査が見ている」と書くときは、**その検査が実際に何を引いているかをコードで確かめる**。ここでは検査(2)の Prisma where が `["refused","undeliverable"]` で、§4-4 の集合（`replied` を含む）と**一致しない**のを見落とした。
...
- **検査(5)は §4-3 と §4-4 のみ**（§4-1/§4-2 は見ない）。理由は Task 5 の前書きに記載。⚠当初「§4-3 のみ」としていたのは誤りで、@codex #374 R1 P1 で訂正した。
- **候補から外れた理由の一覧表示**は作らない。§4 は「絞り込み」であって「診断画面」ではない（`decideResendCandidacy` は理由を返せる形にしてあるので、必要になれば足せる）。

---

## 実装時に計画から変えたところ（実装後に追記・次の作業が空振りしないように）

| # | 計画 | 実際 | 理由 |
|---|---|---|---|
| 1 | Task 4 のテストは `fs.readFileSync` によるソース走査 | **prisma モックへの振る舞いテスト**（`dmExportBatch.create` の `data.resendFilterApplied`／実際に渡った `where.AND`／監査 filters を検証） | 既存 `dm-batches-post-route.test.ts` に prisma モックが揃っており、ソース走査より強く、文言変更で壊れない |
| 2 | Task 5 の CSV route テストは `mockResolvedValueOnce` を2本つなぐ | **クエリの `where` の内容で振り分ける `mockImplementation`** | 検査(5)が走らないケースで2つ目の once が消費されず**次のテストへ漏れて 500 になった**（実際に踏んだ）。呼び出し順に依存しない形にした |
| 3 | `buildResendCandidateWhere` の戻り値は `unknown[]` | **`ResendCandidateWhereFragment[]`（新しい公開型）** | テスト側の `any` が eslint エラーになったのを機に型を付けた。テストが断片を型で読むので、**条件の形を黙って組み替えるとコンパイルで落ちる** |
| 4 | トグルONで `dmStatus` を「送る」に合わせるだけ | **「未送信(0回)」の絞り込みも解除する** | 再送候補は「送付記録が1件以上ある」が条件（§4-2）なので、未送信(0回)と併用すると**必ず0件**になる。提出前の自己レビューで発見 |
| 5 | 画面の配線テストは距離窓 `/setResendOnly\(next\);[\s\S]{0,200}.../` | **onChange 本体を切り出して `toContain`**（＋切り出しが広がったことを長さで検出） | コメントを1行足しただけで窓を超えて落ちた（実際に踏んだ）。切り出しが広がると `handleResetFilters` の同名呼び出しを拾って**素通り**しうるため空振り検出も入れた。**実装を外すとテストが落ちることを実測**済み |

### レビューで見てほしい既知の性質（新しい穴ではないが明示しておく）

- **§4-5 は所有者の他物件の反響を見る**ので、担当外の物件で拒否された相手を持つ物件が候補から消える＝「この所有者はどこかで断られている」という粗い事実が担当者に伝わる。これは PR-A/PR-B が既に採っている方針（控え作成時の terminal 自動除外・件数表示）と同じで、**再送事故の防止のために意図してそうしている**。物件の特定情報は出さない。
- **検査(5) は再試行GETにも掛かる**（初回DL後に送付が入ると digest 一致でも 409）。これは terminal 反響の検査が同じ挙動（R45/R53）なのに合わせたもの。凍結済みの控えでも「配ってはいけなくなったもの」は配らない。
