...
- 定義(すべて AND):
  ...
  5d. ⚠**統合 tx の順序は「source Owner 行の FOR UPDATE を最初に→予約→最後にログ→archive」**(@codex R19/R21 P1): 現行の統合は source Owner 行の更新(archive)が**最後**にあり、先に関連行を掃いてしまうと、掃いた後に作成側が FOR SHARE を取って**新しい source 参照の行を commit** できる(archive はそれを見ない)。統合 tx の**最初に source Owner 行を FOR UPDATE** すれば、作成側の FOR SHARE(5c)と最初から衝突して完全に直列化される。その上で、**未確定バッチ item・drafts(予約)を先に付け替え、`PropertyDmLog` の付け替えを最後**に置く(予約のロック待ち中に作られたログも最後の付け替えが掬う)。
