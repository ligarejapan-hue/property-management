...
- `POST /api/properties/dm-batches/[id]/confirm` body=`{ sentOn: "YYYY-MM-DD" }`。**sentOn は「初回DL(downloadedAt)の JST 暦日以降〜今日以前」のみ受理**(@codex R36/R40 P2 → R51 P2 で下限を強化: 未来日は実在しない送付を作る。下限をバッチ作成日にすると、8/1 作成・8/8 初回DLの控えに 8/1 を書けてしまい、記録が7日分古く=全宛先が7日早く再送候補に戻る。郵送は CSV の入手より前には起き得ず、確定は downloadedAt 必須なので下限は常に存在する。個別記録 POST は過去日可・今日以前のみ)。
...
property_id を nullable 化+FK を ON DELETE SET NULL へ変更(@codex R49 P1: 現行の物件ハード削除はログを Cascade で道連れにし、**拒否/宛先不明の履歴ごと消える**→所有者Oが物件Aで拒否されていても、Aを削除するとOの他物件Bが再送候補に復活する。履歴は所有者側(owner_id+property_dm_log_owners)に残す。物件詳細の履歴表示は propertyId 指定クエリなので無影響・物件が消えた行は所有者横断の除外にだけ効く)。⚠**孤児行の訂正経路を必ず用意する**(@codex R51 P2): 反響の更新・記録の取消はすべて `/api/properties/[id]/...` 配下のため、property_id=null の行に誤った拒否/宛先不明が残ると**辿る手段がなく、その所有者の全物件が永久に候補から消える**。PR-B に admin 専用の孤児記録管理を含める: `GET /api/admin/orphan-dm-logs`(所有者名で検索・一覧)+`PATCH /api/admin/orphan-dm-logs/[logId]`(反響訂正)+`DELETE 同`(取消)。**property_id=null の行のみ受理(それ以外は 404)**・admin のみ・監査は既存 action(dm_reaction_update/dm_sent_record_delete)+detail `orphan:true`・terminal を新たに書くときは Owner 先頭 FOR UPDATE(R47・親行ロックは親が無いので無し)。あわせて物件削除の確認ダイアログに「DM の反響履歴は所有者に引き継がれます」の注意書きを追加。⚠**所有者の紐づけが全く無い行(ownerId=null かつ連関0=個別記録・旧 sale_dm 行)は孤児として残さず、物件削除 tx 内で行ごと削除**する(@codex R52 P1: 物件も所有者も無い行は所有者横断の除外に寄与できず、所有者名検索の孤児管理からも見えない=用途なく残るだけ。現在の PropertyOwner リンクから連関を後付けする補完はしない=「実際に手紙を受け取った宛先」と一致する保証が無い・R33/R34 と同じ判断)
...
- R51(2026-08-08): P1(所有者追加・共有者住所変更がリンク検査をすり抜ける)→**凍結前に住所グループ再計算の完全一致検査**(不一致409)。P2(sentOn下限が作成日)→**初回DLのJST暦日に強化**。P2(孤児terminal行が訂正不能)→**admin専用orphan-dm-logs(検索/訂正/取消・property_id=nullのみ)+物件削除ダイアログに注意書き**。
- R52(2026-08-08): P1(資格検査がno_sendのみ)→**「dmStatus=send かつ isArchived=false」の完全な送付可能条件に拡大**(hold/アーカイブ化も検出)。P1(所有者紐づけゼロの行が無用な孤児になる)→**物件削除tx内で行ごと削除**(連関の後付け補完はしない=R33/R34と同じ判断)。
- R53(2026-08-08): P1(再試行の資格検査がno_send表記のまま)→**初回と同一の資格述語を共通関数1本で両経路に適用**(表記も完全形に統一)。
