...
⚠ **「妥当でない」の判定は狭く取る**(@codex #369 R26 P2)。このリポの `isValidPostalCode` は**日本の7桁**を見るものだが、所有者の郵便番号は**自由入力**で、**海外の郵便番号**(英字を含む・桁数が違う)も入り得る。「日本の形式に合わない＝壊れている」とすると、**正しい海外の番号を宛名から消してしまう**。

→ **抑止するのは「日本の郵便番号を書こうとして失敗している値」だけ**にする(数字とハイフンのみで構成されているのに7桁にならない、など)。**英字や空白を含むなど、明らかに日本の形式ではない値はそのまま刷る**(判断できないものを消さない)。

→ **解決関数は、上の意味で妥当でない郵便番号を「無い」ものとして扱う**:

- **住所はそのまま使う**(宛先は分かっている)
- **郵便番号は空にして刷る**(壊れた番号を刷らない)
- **DB の値はそのまま残す**(§6.2 の候補→修理はこの値を見る)

これで「**修理のために残す**」と「**宛名には出さない**」が両立する。§4.0 のグループ単位の決め方でも、**妥当でない番号は最初から数えない**(食い違いの判定にも入れない)。

**テスト**: 疑わしい郵便番号を取込んだ直後にDMを出すと、**住所は出るが郵便番号は空**であること(直したあとは出ること)。
