...
⚠ **住所の表記を直す補正では、郵便番号を触らない**(@codex #369 R21 P2)。`text-fix` は制御文字の除去・文字化けの修復を行う専用の経路で、**同じ宛先の書き方を直す**操作であって**配達先を変えない**(別の住所への変更はこの経路ではできない)。したがって:

- **`currentZip` はクリアも書き換えもしない**(据え置く)。§6.1 の「住所を書き換えたら郵便番号も決める」の**明示的な例外**とする。§6.1 の目的は「住所と番号が食い違わないこと」であり、宛先が変わらない補正ではペアは崩れない。
- **郵便番号に触れないので `owner_zip` の書込権限は要求しない**(§6.1.1 の暗黙クリアとは別物)。
- ⚠ これは **§6.1 の郵便番号側の例外(表記の修正)と対称**: **表記の補正は、対になるもう一方を据え置く**。

**テスト**: `currentZip` が**非空**の所有者の現住所を text-fix で補正しても、**郵便番号が変わらない**こと。
