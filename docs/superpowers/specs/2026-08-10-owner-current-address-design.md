...
| 経路 | 不正な郵便番号 |
|---|---|
| 手入力・法人情報の反映など**その場で正しい値が決まる**経路 | **null にする**(正しい番号を入れ直せる) |
| **取込**(所有者CSV・ペア取込・手動リンク) | ⚠**そのまま保存する**。品質チェックが `zip_suspicious` として拾い、人が §6.1 の例外経路で直す |

**テスト**: 疑わしい郵便番号を**取込 → 候補に出る → 直せる**まで通す。
