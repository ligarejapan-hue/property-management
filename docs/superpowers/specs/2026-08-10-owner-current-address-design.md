...
**⚠ この規則の目的は「保存されている住所と郵便番号が食い違わないこと」**であって、郵便番号の**書き方**を触ることまで禁じるものではない。よって次を**明示的な例外**とする(@codex #369 R12 P2 = §6.2 の「候補→修正」の流れとの矛盾の解消):

> **例外(表記の整形のみ)**: `currentAddress` が**非空**であるとき、`currentZip` の**表記を整える**補正(`contact-fix`。例「1500001」→「150-0001」・全角→半角)は**単独で許す**。
> これは**どの住所に対応する番号かを変えない**(同じ番号の書き方を直すだけ)ので、ペアは崩れない。
> ⚠**別の番号への置換は例外に含めない**。`currentAddress` が空のときも許さない(対になる住所が無い)。

実装では、この例外を**「整形前後で番号が同値であること」を検査して担保する**(数字だけを取り出して一致するか)。一致しない＝別番号への置換なので拒否する。
