...
- 個別の手直しは既存の draft 編集を使う。**既存 PATCH に貼り付け保存と同一の本文検証ヘルパーを適用**: trim 非空(空白・改行のみは 400=@codex R40 P2)+**タグ検証**=allowlist タグは当該物件の値で展開し、展開後に `{{` が残る本文(未知タグ・綴り違い含む)は **400**(@codex R44 P2: 一括適用側だけ検証しても、手編集で `{{所有者名}}` 等を書けばプレースホルダが印字されたまま郵送される。[同種の穴は全箇所]の原則で貼り付け・一括適用・個別編集の3経路とも同じヘルパー1本に通す)。⚠**既存の draft 編集 PATCH にも property:write を追加**する(@codex R10 P2): 現行は requireSaleDmAccess のみで、一括適用に write 門を付けても**1宛先ずつの編集で迂回できる**(権限を剥奪された作成者が本文を書き換えられる)。同じ結果を生む全経路に同じ門([同種の穴は全箇所]の原則)。
