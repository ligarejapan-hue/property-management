...
- **テンプレ差し替え時の失効(@codex R19 P2)**: 未凍結の variant で body_template を**差し替えて保存**するときは、同一 tx で variant 配下の**未確定 draft の body を全クリア**する(既存の「設定変更で body をクリアする」失効機構と同じ扱い)。これをしないと、旧テンプレAを適用済みの draft が既定の「空本文のみ適用」でBの適用をすり抜け、**確定時に「記録はB・実際の手紙はA」**の食い違いが凍結される。
