...
- **割当(総当たり均等)**: 新純関数 `assignCrossEvenly(recipientIds, dmVariantIds, lpVariantIds, opts)`。
  - LP型 = lp[(k + ⌊k / lcm(n,m)⌋) mod m](ブロック長 lcm(n,m) ごとに 1 ずらす)。任意の接頭辞で組の偏り≤1・LP軸の偏り≤1・DM軸は従来と一致(テストで n,m≤7・r≤100 を総当たり)。
...
- **PC でもスマホでも最適化(発注者要望 2026-09-11)**: 同じ HTML を CSS で切り替える(外部読み込みなし)。**スマホ**(〜767px)=1列・画像は幅いっぱい・画面下に固定の操作バー(「無料査定を申し込む」と「電話」の2つ・親指で届く位置・本文を隠さない余白)。**PC**(768px〜)=中央1列(最大 760px)・ヒーローは 16:9 で大きく・節の写真/図は本文幅・固定バーは出さず本文中のボタンのみ。文字は 16px 以上・タップ領域は 44px 以上。プレビュー帯・凍結・差し込みの規則は両方で同じ。社内プレビューは **PC/スマホの表示切替**(幅 390px と 1000px の枠で同じページを見る)を持つ。
