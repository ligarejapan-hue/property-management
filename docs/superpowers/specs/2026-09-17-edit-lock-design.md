...
- ⚠**答える側(応答器 `answerScreenTokenProbes`)は「画面につき1本・画面の寿命ぶん」**(外部レビュー@codex P1・2026-09-26)。編集を始めているかどうかで絞ってはいけない——空いている(まだ何も編集していない)タブを複製したとき、**元のタブが答えられなければ複製タブは写し取った合言葉を使い続け**、あとで両方が編集を始めても `held_by_self_other_screen` にならず同じ画面として扱われる(D6が黙って成り立たなくなり、片方の保存が版番号で弾かれるまで両方が入力を続けられる)。応答器は個々の編集部品(所有者カード・編集ウィンドウ)より**長生き**でなければならない=部品側・hook 側では張らず、**画面(親)で1本だけ**張って画面を離れるときに片付ける(1本に絞る理由は本数=所有者120人の物件で120本開かないため)。
...
### 外部レビュー(@codex)P1 の反映(2026-09-26・M1 の裁定を差し替え)
