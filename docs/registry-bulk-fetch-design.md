...
> ## ⚠実装状況(2026-08-07)
>
> **本書は「厚い版」＝完全設計。実際に本番稼働しているのは「薄い版」(PR #361・実績83)。**
>
> - **✅実装済み(薄い版・本番LIVE)**: 種類の2択(所有者事項/全部事項)、複数物件の一括取得
>   (ジョブ+項目テーブル、画面からの1件ずつ処理、進捗/中止/再開、50件上限、可視物件フィルタ、
>   冪等キー)。1件ごとの処理は**既存の単発取得 `runRegistryAutoFetch` をそのまま呼ぶ**ため、
>   単発と同じ課金安全(AuditLog二重課金ガード・charged_but_failed・添付必須)を継承する。
> - **⛔見送り(将来の堅牢化ロードマップ・未実装)**: 本書が詳述する**クラッシュ耐性の追加保険**=
>   課金前に書く共通台帳 `registry_charge_attempts`・物件 `registry_lock_token` と全writerの
>   token対応・口座ブレーカー・起動時ゲート・`registry:manage` 管理者代行。これらは**単発取得の
>   課金コアのリファクタを伴う**ため段階的に見送った。将来、実課金の通しテストを経てさらに堅く
>   する段でこの章が着手の地図になる。
> - この文書は**設計の記録＋将来の地図**として残す。§各章の「schema」「安全弁」等は厚い版の
>   目標であって、薄い版の現状とは一致しない箇所がある(上の切り分けが正)。
...
registry_fetch_jobs:      id / status(pending|processing|paused|completed|cancelled)
                          / certificate_type / requested_by / counts(=保存用の
                          全体値。⚠進捗/詳細の応答では返さず「可視な項目集合」から
                          都度計算し直す=伏せた物件の結果を数字から漏らさない)
                          / timestamps
...
- ⚠**項目の掴みに加えて、ジョブ単位でも「同時に1件だけ」を強制する**
  ...
  **課金境界の直前(onBeforeCharge の中)でジョブが paused/cancelled になって
  ...
  ⚠**再確認は canAccessPropertyRecord だけで済ませず、課金クリック直前に
  APIの完全な認可ゲートを丸ごと再評価する**(@codex 設計指摘 P1)。
  canAccessPropertyRecord はセッションの役割と担当だけを見るため、
  **順番待ちの間に registry:auto_fetch や property:read を剥奪された/
  アカウントが無効化された/役割が降格された**場合を見落とし、権限を失った
  リクエストが課金に進み得る。課金境界の直前で**現在のユーザー状態と権限を
  DBから読み直し**、取得APIの入口と同じ権限判定(registry:auto_fetch AND
  property:read + 有効アカウント + canAccessPropertyRecord)を**もう一度全部
  通す**。1つでも欠けたら押さず pending へ戻す
...
- 権限は **registry:auto_fetch AND property:read** を要求し、さらに**項目ごとに
  ...
  ⚠**集計カウントも「見えている項目」から毎回計算し直す**(@codex 設計指摘 P2)。
  スキーマの `counts`(done/failed/skipped 等)は**保存用の全体値**なので、
  項目を伏せても保存済みの合計をそのまま返すと、**伏せたはずの物件の処理結果
  (成功/失敗した事実)が数字から漏れる**(1件ジョブなら特に露骨・混在ジョブでも
  合計から推測できる)。進捗/詳細の各応答では、保存済み `counts` を返さず、
  **その応答で可視な項目集合だけを数えた値**を返す(または合計を省く)。
