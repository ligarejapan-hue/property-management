...
- [ ] **JPEG（ダミー GPS / EXIF 付き）= 標準**: PC 上の通常の JPEG（無地の被写体）に exiftool で
      **架空・非実在・非センシティブな検証用のダミー GPS と EXIF** を **EXIF グループ指定**で注入する
      （例: GPS は実在しない座標、Make / Model / DateTimeOriginal も架空値。値は記録に残さない）。
      **実機カメラでの実 GPS 撮影は標準手順にしない**（本番検証中に実 GPS EXIF 入り写真を作成・転送すると
      GPS / PII ルールと矛盾するため）。strip は server 側処理で端末非依存のため、ダミー GPS JPEG で十分に検証できる
...
> **実機カメラでの実 GPS 撮影は「例外扱い」**（通常手順ではない）。どうしても実機の実 GPS 挙動を
> 補足確認する場合に限り、以下を**すべて**満たすこと:
> - 事前承認を得る
> - 非センシティブな検証場所のみ（自宅・取引先・調査対象等の実在地で撮影しない）
> - 撮影した写真・EXIF・GPS 値をチャット / ログ / PR / Issue / スクリーンショットに貼らない
> - 検証後にただちに削除する（端末・PC に保管しない）
> - 標準手順（ダミー GPS 注入 JPEG）の代替ではなく、あくまで例外的な補足確認とする
...
| # | 入力 | 期待結果 |
|---|---|---|
| 6-1 | iPhone での JPEG アップロード（§2 のダミー GPS / EXIF 注入 JPEG をギャラリー経由で使用。実 GPS 撮影は不要） | アップロード成功。after: **GPS / Make / Model / DateTimeOriginal / シリアル / MakerNote / XMP がすべて無い**。Orientation のみ残り得る（値 1〜8）。ファイルサイズは原本より小さい（増加したら所見として報告） |
| 6-2 | Android での JPEG アップロード（同上） | 同上 |
...
> **サーバー側 422 のうち HEIC / 8MB 超の回帰確認は直接 API POST
> （curl / DevTools fetch で FormData 送信）で行う** — server invariant は不変。
> ※ malformed（8MB 以内・MIME が JPEG/PNG/WebP）は**無変換 pass-through で
> 今も UI からサーバーへ届き 422 になる**（端末内 decode は変換対象のみ実行）。
...
| 7-3 | malformed（ゼロ列 `.jpg`・8MB 以内・MIME=image/jpeg） | **従来どおりサーバー 422**（8MB 以内の JPEG は無変換 pass-through のため POST は発生する）。画面「入力内容に誤りがあります。」・DevTools「画像ファイルを処理できませんでした。」。**写真は保存されない**（一覧に増えない） |
...
| 7-5 | 8MB + 1 byte の実画像 JPEG | **成功**。端末内で縮小され、レスポンスの `fileSize` が 8MB 以下・`mimeType` が `image/jpeg`。（8MB 超は変換対象になるため、**非画像バイトの巨大 `.jpg` だけは**端末内 decode 失敗で送信されず「この写真は読み込めませんでした…」表示になる） |
...
> **合成テストが green でも、実機検証の NG は NG として扱う。** 実機検証は、合成 fixture では
> 拾えない問題（iOS / Android の MIME 変換、カメラ / ギャラリー挙動、ブラウザ挙動、
> storage / proxy 挙動、production 設定差分、GPS 残存、Orientation 崩れ、422 時の副作用 等）を
> 見つけるために行う。**GPS 残存・Orientation 不具合・422 時の副作用（写真行が増える等）・
> 失敗したのに写真が増える等が観測されたら、合成テスト green でも NG**。
>
> - **環境要因と決めつけない**（「テストが green だから端末側のせい」と先に結論しない）
> - コード / ブラウザ / 端末 / storage・proxy / 設定差分を**並列に**切り分ける（どれか 1 つを先に疑わない）
> - **原因修正に入る前に**、項目 ID・端末 / OS / ブラウザ・期待値・実結果・DevTools 情報
>   （HTTP status・サーバ文言・mimeType 等）を §12 の形式で整理して**いったん停止**し、報告する
>
> （HEIC→JPEG 自動変換のように「仕様どおりの環境差」だと**確認できた**場合は §5 / §7 のとおり「所見」扱い。
> ただし確認前から環境差と決めつけない。）
