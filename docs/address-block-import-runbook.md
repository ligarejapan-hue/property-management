...
- ⚠**取込エリアの境界の外**(例: 未取込の隣県側)でピンを打つと、境界越しに100m以内の点を拾って隣県の住所が出ることがある(**営業エリアの都道府県は一括で取り込む**のが安全)。提案値は必ず確認する運用
- 取込時の安全装置: 不正行1%超のファイルは停止(--allow-skipped で強行) / 既存の半分未満へ縮む置換は停止(--allow-shrink で強行) / 地番の自動セットは点から50m以内のときのみ
