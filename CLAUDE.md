...
- 禁止する記述の例:
  - ツール呼び出し用の XML 風タグ（`invoke` / `parameter` 形式。Bash・Read・Edit・Write・Glob・Grep など全ツール）を、実際の呼び出しを行わず応答本文にテキストとして書き出すこと
  - `call` で始まるツール呼び出し風・コマンド実行風の擬似表記
  <!-- 注: 禁止対象タグの「生の並び」を具体例として本ファイルに列挙しない。生テンプレがコンテキストに常駐するとモデルがそれを再現し、本ルールが防ぎたい疑似呼び出し（ツール呼び出しがテキストとして漏れ未実行になる事故）をかえって誘発するため。2026-06-09 セッション 20-B で実害を確認し、具体タグ列の列挙からこの記述へ置換した。 -->
